...
def menu(x, y, w, h):
    ...
    at(x + 2, y + 13, "[6] multi send from file", c['w'])
...
async def multi_from_file():
    ...
    x = (cr[0] - w) // 2
    y = 2
    box(x, y, w, hb, "multi send from file")
    at(x + 2, y + 2, "enter path to txt file:", c['y'])
    at(x + 2, y + 3, "type [esc] to cancel", c['c'])
    at(x + 2, y + 4, "─" * (w - 4), c['w'])
    
    file_path = await ainp(x + 2, y + 5)
    if not file_path or file_path.lower() == 'esc':
        return
    ...
    try:
        ...
        at(x + 2, y + 7, "file not found!", c['bgr'] + c['w'])
        await awaitkey()
        return
    except Exception as e:
        at(x + 2, y + 7, f"error reading file: {e}", c['bgr'] + c['w'])
        await awaitkey()
        return
    ...
    # This part is updated to only use spaces as separator
    wallet_line_re = re.compile(r"^\s*(oct[1-9A-HJ-NP-Za-km-z]{44})\s+(\d*\.?\d+)\s*$")
    ...
    for line in lines:
        ...
        match = wallet_line_re.match(line)
        
        if match:
            addr_to, amount_str = match.groups()
            a = float(amount_str)
            if a > 0:
                rcp.append((addr_to, a))
                tot += a
                at(x + 2, ly, f"✓ {addr_to[:20]}... {a:.6f}", c['g'])
                ly += 1
            else:
                at(x + 2, ly, f"✗ zero amount: {line[:30]}...", c['R'])
                ly += 1
        else:
            at(x + 2, ly, f"✗ invalid line: {line[:30]}...", c['R'])
            ly += 1
        
        if ly >= y + hb - 8:
            at(x+2, ly, "too many lines to display...", c['y'])
            break

    if not rcp:
        at(x + 2, ly, "no valid recipients found in file.", c['R'])
        await awaitkey()
        return
